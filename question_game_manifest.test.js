...
assert.strictEqual(manifest.length,85,'manifest count');
assert.strictEqual(new Set(manifest.map(x=>x.id)).size,85,'manifest duplicate ids');
...
assert.deepStrictEqual(counts,{fixed500:31,generated500:13,existingRich:4,nonQuestion:37});
