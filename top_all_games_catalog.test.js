...
if(games.length!==84){
  console.error(`Expected 84 TOP cards, got ${games.length}`);
  process.exit(1);
}
const index=fs.readFileSync(path.join(root,'index.html'),'utf8');
if(!index.includes('<span id="gameCount">84</span>')){
  console.error('index.html gameCount is not 84');
  process.exit(1);
}
console.log('PASS: all 84 TOP game cards are registered');
