...
assert.strictEqual(catalogEntryCount(), 85, 'catalog must contain 85 games after exposing all active individual games');
...
assert.match(index,/<span id="gameCount">85<\/span>/,'visible game count must be 85');
assert.ok(index.includes('id="embeddedGameCatalog"'),'TOP must embed the 85-game catalog to avoid stale external catalog cache');
assert.ok(!/<script[^>]+src="assets\/game_catalog\.js/i.test(index),'TOP must not depend on external game_catalog.js');
console.log('PASS integrated flag game within unified 85 games');
