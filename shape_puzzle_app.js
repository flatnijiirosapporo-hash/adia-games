...
const TRAY_CELL_PX=46;
const SNAP_RADIUS_CELLS=1.25;
const DROP_MARGIN_CELLS=0.9;
const DRAG_LIFT_PX=18;
...
function onPiecePointerDown(e){if(!activeResult)return;e.preventDefault();const token=e.currentTarget,id=token.dataset.pieceId,wasSelected=selectedPieceId===id;selectedPieceId=id;renderPreview();const rect=token.getBoundingClientRect(),cells=rotatedFor(id),b=bounds(cells);drag={id,token,pointerId:e.pointerId,startX:e.clientX,startY:e.clientY,lastX:e.clientX,lastY:e.clientY,moved:false,wasSelected:wasSelected,source:token.dataset.source,oldPlacement:placements[id]?{...placements[id]}:null,pieceWidthCells:b.w,pieceHeightCells:b.h,grabRatioX:Math.min(1,Math.max(0,(e.clientX-rect.left)/Math.max(1,rect.width))),grabRatioY:Math.min(1,Math.max(0,(e.clientY-rect.top)/Math.max(1,rect.height)))};token.setPointerCapture?.(e.pointerId);token.classList.add('dragging','selected');markMeaningfulAction()}
function onGlobalPointerMove(e){if(!drag||e.pointerId!==drag.pointerId)return;if(e.cancelable)e.preventDefault();const dx=e.clientX-drag.startX,dy=e.clientY-drag.startY;if(Math.hypot(dx,dy)>DRAG_THRESHOLD_PX&&!drag.moved){drag.moved=true;activeResult.moves+=1;drag.token.classList.add('drag-lift')}drag.lastX=e.clientX;drag.lastY=e.clientY;if(drag.moved)drag.token.style.transform=`translate(${dx}px,${dy-DRAG_LIFT_PX}px) scale(1.06)`}
function onGlobalPointerUp(e){if(!drag||e.pointerId!==drag.pointerId)return;const d=drag;drag=null;d.token.releasePointerCapture?.(e.pointerId);d.token.classList.remove('dragging','drag-lift');d.token.style.transform='';
  if(!d.moved){if(d.wasSelected)rotateSelectedPiece();else{renderPuzzle();markMeaningfulAction()}return}
  const board=$('shapeBoard'),br=board.getBoundingClientRect(),cellSize=br.width/currentPuzzle.boardWidth,margin=cellSize*DROP_MARGIN_CELLS;
  if(e.clientX>=br.left-margin&&e.clientX<=br.right+margin&&e.clientY>=br.top-margin&&e.clientY<=br.bottom+margin){
    const rawX=(e.clientX-br.left)/cellSize-d.grabRatioX*d.pieceWidthCells;
    const rawY=(e.clientY-DRAG_LIFT_PX-br.top)/cellSize-d.grabRatioY*d.pieceHeightCells;
    const piece=pieceFor(d.id),candidate=core.nearestValidPlacement(piece,pieceState[d.id].rotation,rawX,rawY,currentPuzzle,placements,SNAP_RADIUS_CELLS);
    if(candidate){placements[d.id]=candidate;if(invalidFlags[d.id]){activeResult.recoveredInvalidCount+=1;invalidFlags[d.id]=false};setMessage('');renderPuzzle();markMeaningfulAction();checkSolved();return}
  }
  activeResult.invalidPlacements+=1;invalidFlags[d.id]=true;if(d.oldPlacement)placements[d.id]=d.oldPlacement;else delete placements[d.id];setMessage('そこには はいらないみたい。べつのばしょも ためしてみよう！');d.token.classList.add('invalid-drop');markMeaningfulAction();setTimeout(()=>{if(currentPuzzle&&activeResult)renderPuzzle()},220)
}
